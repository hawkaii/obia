...
const (
	...
	fieldPriority = 3
	fieldStatus   = 4
	fieldPush     = 5 // only when showPush
)
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		if m.showPush && m.focusIndex != fieldSummary && m.focusIndex != fieldTarget && m.focusIndex != fieldDueDate {
			switch k {
			case "y":
				m.push = 1
				return m, nil
...
func (m Model) View() string {
	...
	b.WriteString(m.renderField("Due", m.dueDate.View(), m.focusIndex == fieldDueDate))
	b.WriteString("\n")
...
func (m Model) GetDue() *time.Time {
	val := strings.TrimSpace(m.dueDate.Value())
	if val == "" {
		return nil
	}
